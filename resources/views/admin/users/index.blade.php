...
{{-- On récupère le nom de la route actuelle --}}
@php
$route = request()->route()->getName();
@endphp

{{-- Si la route est index le titre sera "Tous les utilisateurs" si elle est bin ce sera "Utilisateurs supprimés" --}}
@if (str_contains($route,'index'))
    @section('title','Tous les utilisateurs')
@elseif (str_contains($route,'bin'))
    ...
    <div class="d-flex justify-content-start mt-1 mb-4">
        ...
        <div class="">
            <form action="" method="get" class="container d-flex gap-2">
                <input type="text" placeholder="Utilisateur..." class="form-control" name="name" value="{{ $input['name'] ?? '' }}">
                <button class="btn btn-success btn-sm">Chercher</button>
            </form>
        </div>
        {{-- Si on est sur la route bin afficher le bouton pour supprimer tous les utilisateurs --}}
        @if (str_contains($route,'bin'))
            <div class="ms-auto">
